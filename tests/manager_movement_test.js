...
// JSDOM must be required and set up BEFORE any application modules are imported.
...
// Mock the global Offers object that the simulation script expects.
// This prevents the "read-only" error from trying to modify an imported module,
// as the simulation code calls `window.Offers.showJobOffersPopup`.
global.window.Offers = {
  showJobOffersPopup: () => { /* Mocked: do nothing to prevent UI popup */ },
  showTransferNewsPopup: (transfers, cb) => { if (typeof cb === 'function') cb(); },
};

await import('../src/core/simulation.js'); // Imports for side-effects, populates window.Simulation
const logger = getLogger();
  ...
  // O script simulation.js já popula window.Simulation, não é preciso atribuir.
