...
try {
  logger.info('Iniciando teste de Agentes Livres (generateFreeAgents)...');
  ...
  const freeAgents = window.generateFreeAgents(allDivisions, { probability: 1.0, maxPerClub: 2 });
  ...
  logger.info('✔️  ASSERT 1: O limite máximo por clube (maxPerClub: 2) foi respeitado.');
  ...
  logger.info('✔️  ASSERT 2: Os jogadores foram corretamente removidos dos plantéis originais.');
  ...
  logger.info('✔️  ASSERT 3: As propriedades contratuais dos agentes livres (0 anos) estão corretas.\n');

  logger.info('Teste de Agentes Livres: PASSOU com sucesso!');
  process.exit(0);

} catch (err) {
  logger.error('Teste de Agentes Livres FALHOU:', err && err.stack ? err.stack : err.message);
  process.exit(2);
}