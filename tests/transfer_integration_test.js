...
console.log('Seller budget:', seller.budget);
console.log('Buyer budget: ', buyer.budget);
console.log('Seller players:', seller.team.players.map(p=>p.name));
console.log('Buyer players: ', buyer.team.players.map(p=>p.name));
...
function executePendingPurchase(pendingIdx, offerSalary = 30000, offerYears = 1) {
  ...
  if (sellerClub && sellerClub.team && Array.isArray(sellerClub.team.players)) {
    realPlayer = sellerClub.team.players.find(pp => (pp && pp.id && pl.id && pp.id === pl.id) || (pp && pp.name === pl.name));
  }
  if (!realPlayer) {
    for (let c of window.ALL_CLUBS || []) {
      ...
      if (found) { realPlayer = found; sellerClub = c; break; }
    }
  }
  ...
  if (realPlayer && sellerClub && sellerClub.team && Array.isArray(sellerClub.team.players)) {
    const ridx = sellerClub.team.players.findIndex(pp => pp === realPlayer || (pp && pl.id && pp.id === pl.id) || (pp && pp.name === pl.name));
...
try {
  ...
  assert.strictEqual(seller.team.players.find(p=>p.id === 'p_test_001'), undefined, 'Seller should no longer have the player');
  assert.strictEqual(window.PENDING_RELEASES.length, 0, 'Pending releases should be empty');
  assert.strictEqual(buyer.budget, 1000000 - 200000, 'Buyer budget should have decreased by the leaving fee');
  assert.strictEqual(seller.budget, 500000 + 200000, 'Seller budget should have increased by the leaving fee');
