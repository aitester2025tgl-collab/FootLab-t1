module.exports = require('../src/legacy_ui/hub');
...
(function () {
  ...
  function initHubUI() {
    ...
    try {
      ...
      const count = (E.playerClub && E.playerClub.team && Array.isArray(E.playerClub.team.players))
    ...
    try {
      ...
      if (coachEl) {
        // prefer window.coachName, then playerClub.coach, then empty
        const name = (window.coachName && String(window.coachName).trim()) ||
          (E.playerClub && E.playerClub.coach) ||
          (E && E.playerClub && E.playerClub.team && E.playerClub.team.coach) ||
          '';
  ...
  function showBuyFreePlayerMenu(pl, rawFreeAgents, idxInFiltered) {
    try {
      ...
      overlay.innerHTML = '';
      const box = document.createElement('div');
      // use standard subs-panel + transfer root classes for consistent sizing and theme
      box.className = 'subs-panel transfer-overlay-root';
      // outer overlay: use standard opening-game colors (blue gradient) for consistent outer look
      try {
        overlay.style.background = 'linear-gradient(180deg, #081c3a 0%, #123a6d 60%)';
      ...
      // inner panel: use player's (previous) club/team colors when available so the card feels like the player's team
      try {
        const clubRef = pl.originalClubRef || pl.club || null;
        const teamBg = (clubRef && clubRef.team && clubRef.team.bgColor) || (pl.team && pl.team.bgColor) || '#2e2e2e';
        const teamSec = (clubRef && clubRef.team && clubRef.team.color) || (pl.team && pl.team.color) || '#ffffff';
        const rgb = (window.ColorUtils && window.ColorUtils.hexToRgb && window.ColorUtils.hexToRgb(teamBg)) || [34, 34, 34];
        const panelBg = `linear-gradient(rgba(0,0,0,0.48), rgba(0,0,0,0.28)), rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.9)`;
        const fg = (window.ColorUtils && window.ColorUtils.getReadableTextColor && window.ColorUtils.getReadableTextColor(teamBg, teamSec)) || '#fff';
        const card = box.querySelector('.transfer-card') || box;
  ...
  window.Offers.showPendingReleasesPopup = function (cb) {
    ...
    // outer overlay: use opening-game blue gradient for consistent outer look
    try {
      overlay.style.background = 'linear-gradient(180deg, #081c3a 0%, #123a6d 60%)';
      overlay.style.color = '#ffffff';
    } catch (e) {
    ...
    overlay.appendChild(box);
    // style inner transfer-card to use player's team color if available
    try {
      const p0 = (window.PENDING_RELEASES || [])[currentIdx] || null;
      const clubRef0 = p0 && (p0.originalClubRef || p0.club) ? (p0.originalClubRef || p0.club) : null;
      const teamBg0 = (clubRef0 && clubRef0.team && clubRef0.team.bgColor) || (p0 && p0.team && p0.team.bgColor) || '#2e2e2e';
      const teamSec0 = (clubRef0 && clubRef0.team && clubRef0.team.color) || (p0 && p0.team && p0.team.color) || '#ffffff';
      const rgb0 = (window.ColorUtils && window.ColorUtils.hexToRgb && window.ColorUtils.hexToRgb(teamBg0)) || [34, 34, 34];
      const panelBg0 = `linear-gradient(rgba(0,0,0,0.48), rgba(0,0,0,0.28)), rgba(${rgb0[0]}, ${rgb0[1]}, ${rgb0[2]}, 0.9)`;
      const fg0 = (window.ColorUtils && window.ColorUtils.getReadableTextColor && window.ColorUtils.getReadableTextColor(teamBg0, teamSec0)) || '#fff';
      const card0 = box.querySelector('.transfer-card') || box;
      card0.style.background = panelBg0;
      card0.style.color = fg0;
    } catch (e) {
      /* ignore */
    }
    ...
    setTimeout(() => {
      const renderIdx = (i) => {
        ...
        box.innerHTML = makeViewerHtml(currentIdx);
        // update inner card color to match the current player's team
        try {
          const p = (window.PENDING_RELEASES || [])[currentIdx] || null;
          const clubRef = p && (p.originalClubRef || p.club) ? (p.originalClubRef || p.club) : null;
          const teamBg = (clubRef && clubRef.team && clubRef.team.bgColor) || (p && p.team && p.team.bgColor) || '#2e2e2e';
          const teamSec = (clubRef && clubRef.team && clubRef.team.color) || (p && p.team && p.team.color) || '#ffffff';
          const rgb = (window.ColorUtils && window.ColorUtils.hexToRgb && window.ColorUtils.hexToRgb(teamBg)) || [34, 34, 34];
          const panelBg = `linear-gradient(rgba(0,0,0,0.48), rgba(0,0,0,0.28)), rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.9)`;
          const fg = (window.ColorUtils && window.ColorUtils.getReadableTextColor && window.ColorUtils.getReadableTextColor(teamBg, teamSec)) || '#fff';
          const card = box.querySelector('.transfer-card') || box;
      ...
      const attachButtons = () => {
        ...
        // Headless/backwards-compat: if `prompt` exists and we're in Node, simulate legacy flow via the inline handler
        // (no visual toggle button in the overlay anymore; users interact with the inline fields)
        try {
          if (typeof prompt === 'function' && typeof process !== 'undefined') {
            // simulate legacy prompt/confirm sequence at attachment time so tests clicking inline button will work
        ...
        // handle the actual propose action from the inline input/button
        const doPropose = () => {
          const pl = (window.PENDING_RELEASES || [])[currentIdx];
        ...
        // attach inline propose button handler
        const doBtn = document.getElementById('offersDoProposeBtn');
