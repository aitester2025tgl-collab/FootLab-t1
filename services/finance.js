...
(function () {
  function computeMatchAttendance(match) {
    ...
    const homePlayers = Array.isArray(match.homePlayers)
      ? match.homePlayers
      : homeClub.team && Array.isArray(homeClub.team.players)
        ? homeClub.team.players
        : [];
    const awayPlayers = Array.isArray(match.awayPlayers)
      ? match.awayPlayers
      : awayClub && awayClub.team && Array.isArray(awayClub.team.players)
        ? awayClub.team.players
        : [];

    const homeSkill = homePlayers.length
      ? homePlayers.reduce((s, p) => s + (p.skill || 0), 0) / homePlayers.length
      : 50;
    const awaySkill = awayPlayers.length
      ? awayPlayers.reduce((s, p) => s + (p.skill || 0), 0) / awayPlayers.length
      : 50;
    ...
    const rawCap = Number(
      (homeClub &&
        (homeClub.stadiumCapacity || (homeClub.team && homeClub.team.stadiumCapacity))) ||
        (homeClub && homeClub.stadium) ||
        10000
    );
    ...
    const homeMembers = Number(
      (homeClub && (homeClub.members || (homeClub.team && homeClub.team.members))) ||
        (homeClub && homeClub.supporters) ||
        (homeClub && homeClub.fans) ||
        1000
    );
    ...
    const perfBonus =
      homeClub && typeof homeClub.points === 'number'
        ? Math.min(0.15, (homeClub.points || 0) / 500)
        : 0;
    ...
    const homePos =
      homeClub && Number(homeClub.currentLeaguePosition)
        ? Number(homeClub.currentLeaguePosition)
        : homeClub && homeClub.team && Number(homeClub.team.currentLeaguePosition)
          ? Number(homeClub.team.currentLeaguePosition)
          : homeClub && Number(homeClub.leaguePosition)
            ? Number(homeClub.leaguePosition)
            : null;
    const awayPos =
      awayClub && Number(awayClub.currentLeaguePosition)
        ? Number(awayClub.currentLeaguePosition)
        : awayClub && awayClub.team && Number(awayClub.team.currentLeaguePosition)
          ? Number(awayClub.team.currentLeaguePosition)
          : awayClub && Number(awayClub.leaguePosition)
            ? Number(awayClub.leaguePosition)
            : null;
    const highProfile =
      (homePos && homePos >= 1 && homePos <= 4) || (awayPos && awayPos >= 1 && awayPos <= 4);
    ...
    const divisionBaseFraction = { 1: 0.5, 2: 0.35, 3: 0.2, 4: 0.1 };
    const baseMin = Math.max(
      50,
      Math.round(effectiveCap * (divisionBaseFraction[clubDivision] || 0.1))
    );
    ...
    try {
      ...
      if (
        ...
        window.allDivisions &&
        Array.isArray(window.allDivisions[clubDivision - 1])
      ) {
        divisionSize = window.allDivisions[clubDivision - 1].length || divisionSize;
      }

      const rankPercentile = function (club) {
        if (!club) return 0.5;
    ...
    let attendance = Math.round(
      Math.max(baseMin, Math.min(effectiveCap, Math.max(attendanceEstimate, baseMin)))
    );
  ...
  function computeTargetTicketPrice(club) {
    ...
    const avgSkill = players.length
      ? players.reduce((s, p) => s + (p.skill || 0), 0) / players.length
      : 50;
        ...
        ? 1 -
          (pos - 1) /
            Math.max(
              ...
              club.division
                ? (window.allDivisions &&
                    window.allDivisions[club.division - 1] &&
                    window.allDivisions[club.division - 1].length) ||
                    18
                : 18
            )
        : 0.5;
    ...
    const target = Math.round(base * (1 + membersFactor * 0.3 + skillFactor + posFactor * 0.15));
  ...
  function negotiatePlayerContract(club, playerIdOrIndex, proposedMonthlySalary, years) {
    ...
    // Enforce player's minimum expected salary when available. If proposed salary
    // is below the player's `minContract` (or `minMonthly`/`minSalary`), reject early.
    try {
      const targetPlayer = player || null;
      const minC = targetPlayer
        ? Number(targetPlayer.minContract || targetPlayer.minMonthly || targetPlayer.minSalary || 0)
        : 0;
      if (minC && proposedMonthlySalary < minC) {
        return { accepted: false, acceptProb: 0 };
      }
    } catch (e) {
      // defensive: continue negotiation if something unexpected happened
    }
    ...
    if (salaryRatio >= 1) {
      ...
      let requiredRaise = 0.1; // default ~10% for similar clubs
      if (relative < -10) {
      ...
      if (raisePct >= requiredRaise) {
        ...
        const penalty = Math.min(0.3, shortfall * 0.8);
