...
export function showSeasonSummaryOverlay(summary, cb) {
  try {
    ...
    if (summary.topScorers && summary.topScorers.length) {
      sections.push(
        '<section><h3>Artilheiros</h3><ol>' +
          summary.topScorers.map((s) => `<li>${s.player} (${s.club}) - ${s.goals}</li>`).join('') +
          '</ol></section>'
      );
    }
    if (summary.assists && summary.assists.length) {
      sections.push(
        '<section><h3>Assistências</h3><ol>' +
          summary.assists.map((s) => `<li>${s.player} (${s.club}) - ${s.assists}</li>`).join('') +
          '</ol></section>'
      );
    }
    if (summary.awards && summary.awards.length) {
      sections.push(
        '<section><h3>Prêmios</h3><ul>' +
          summary.awards.map((a) => `<li>${a.title} - ${a.player} (${a.club})</li>`).join('') +
          '</ul></section>'
      );
    }
