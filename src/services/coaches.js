...
window.COACH_SERVICE =
  ...
  (function () {
    ...
    function assignCoachToTeam(coachName, teamName, clubObj) {
      ...
      try {
        if (!_suppressEvents && typeof window.onCoachHired === 'function')
          window.onCoachHired({ coach: c, teamName, club: clubObj });
      } catch (e) {
        /* ignore */
      }
      return c;
    ...
    function recordMatchResult(teamName, result) {
      ...
      if (games >= 10) {
        ...
        try {
          const club =
            (window.ALL_CLUBS || []).find((c) => c.team && c.team.name === teamName) || null;
          ...
          if (winRate + 0.05 < expected) {
            ...
            try {
              if (window.UNEMPLOYED_COACHES && Array.isArray(window.UNEMPLOYED_COACHES)) {
                if (!window.UNEMPLOYED_COACHES.includes(coach.name))
                  window.UNEMPLOYED_COACHES.push(coach.name);
              }
            } catch (e) {
              /* ignore */
            }
            // expose an event if consumer wants to react
            if (typeof window.onCoachFired === 'function')
    ...
    function hireFromUnemployedForClub(clubObj) {
      ...
      const candidates = Object.values(coachesByName)
        .filter((c) => c.unemployed)
    ...
    function listAll() {
      return Object.values(coachesByName);
    }
...
try {
  if (window.REAL_COACHES) {
    Object.keys(window.REAL_COACHES).forEach((teamName) => {
      ...
      if (coachName)
        window.COACH_SERVICE.registerCoach(coachName, { team: teamName, unemployed: false });
    });
  }
} catch (e) {
  /* ignore */
