  ...
  try {
    ...
    if (storageValue) {
      await page.evaluateOnNewDocument((v) => {
        try {
          localStorage.setItem('footlab_t1_save_snapshot', v);
        } catch (e) {}
        try {
          localStorage.setItem('elifoot_save_snapshot', v);
        } catch (e) {}
      }, storageValue);
    ...
    const diag = await page.evaluate(() => {
      ...
      const fin =
        document.querySelector('#hub-finance-panel') ||
        document.querySelector('.hub-menu-footer') ||
        null;
      if (fin) {
    ...
    const report = {
      ...
      thresholds: {
        maxButtonHeightPx: 40,
        maxLeftColumnWidthPx: 260,
        financeShouldBeVisible: true,
      },
