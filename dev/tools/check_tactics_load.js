...
(async () => {
  ...
  try {
    ...
    try {
      ...
      console.log('No dev server detected — starting local dev/dev-server.js');
      serverProc = cp.spawn(process.execPath, ['dev/dev-server.js'], {
        cwd: process.cwd(),
    ...
    page.on('response', (res) => {
      if (res.status() >= 400)
        logs.push({ type: 'response-error', text: res.status() + ' ' + res.url() });
    });
    try {
