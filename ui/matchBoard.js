...
(function(){
    ...
    function luminance(rgb){
        ...
        if (!rgb) return 0;
        const srgb = rgb.map(v=>{ const s = v/255; return s <= 0.03928 ? s/12.92 : Math.pow((s+0.055)/1.055,2.4); });
    ...
    function getReadableTextColor(bgHex, preferredHex){
        ...
        }catch(e){ return preferredHex || '#fff'; }
    }

    function renderInitialMatchBoard(allDivisions) {
        const allMatches = window.Elifoot.currentRoundMatches;
        if (!allMatches) return;

        try {
            const player = window.Elifoot.playerClub;
            ...
            if (window.Elifoot && window.Elifoot.GAME_NAME && typeof document !== 'undefined') {
                document.title = `${window.Elifoot.GAME_NAME} — ${player && player.team ? player.team.name : ''}`;
            }
        } catch (err) { /* ignore */ }
                ...
                try {
                    matches.forEach(match => {
                        ...
                        try {
                            ...
                            else if (window.Finance && typeof window.Finance.computeMatchAttendance === 'function') {
                                const a = window.Finance.computeMatchAttendance(match);
        ...
        try { adjustMatchBoardSizing(); } catch(e) { /* ignore */ }
    }

    function updateMatchBoardLine(matchIndex, matchResult) {
        if (window.DEBUG_MATCH_SIM) {
            try { console.log('DBG updateMatchBoardLine called', { matchIndex, hasGoals: Array.isArray(matchResult.goals) ? matchResult.goals.length : 0 }); } catch(e){}
        }
        const lineElement = document.getElementById(`match-line-${matchIndex}`);
        if (!lineElement) {
            if (window.DEBUG_MATCH_SIM) try { console.warn('DBG updateMatchBoardLine: element not found for index', matchIndex); } catch(e){}
            return;
        }
        ...
        try {
            ...
            if (typeof matchResult.attendance !== 'undefined') {
                ...
                try { attendance = window.Finance.computeMatchAttendance(matchResult).attendance; } catch(e) { attendance = null; }
            }
            if (specEl) {
                specEl.textContent = attendance === null || typeof attendance === 'undefined' ? '—' : `${attendance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}`;
            }
        } catch(e) { /* ignore UI spectator errors */ }
    }
    ...
    window.addEventListener('resize', function(){ try{ adjustMatchBoardSizing(); } catch(e){} });
