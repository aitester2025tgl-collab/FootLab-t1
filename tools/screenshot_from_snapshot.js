...
(async () => {
  try {
    ...
    // inject localStorage before any script executes
    ...
    await page.evaluateOnNewDocument(() => {
      try {
        ...
        const toWrap = [
          ...
          {
            path: 'Elifoot.Overlays.showHalfTimeSubsOverlay',
    ...
    if (!allMenus) {
      ...
      const seq = [
        ...
        {
          name: 'half-time-subs',
          eval: "if (window.Elifoot && window.Elifoot.Overlays && typeof window.Elifoot.Overlays.showHalfTimeSubsOverlay === 'function') { try { var match = (window.currentRoundMatches && window.currentRoundMatches[0]) || null; window.Elifoot.Overlays.showHalfTimeSubsOverlay(window.playerClub || null, match, function(){}); } catch(e){} }",
          overlay: true,
        },
        {
          name: 'offers-popup',
