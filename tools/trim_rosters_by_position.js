...
function parseFormation(str){
  ...
  const partsFlag = String(str||'4-3-3').split(':').map(s=>s.trim());
...
function parseFormations(arg){
  ...
  return String(arg||'4-3-3').split(',').map(s=>s.trim()).filter(Boolean).map(parseFormation);
}
...
const targetTotal = Number((argv.find(a=> a.startsWith('--target='))||'--target=28').split('=')[1]) || 28;
const minGK = Number((argv.find(a=> a.startsWith('--mingk='))||'--mingk=2').split('=')[1]) || 2;
const minST = Number((argv.find(a=> a.startsWith('--minst='))||'--minst=2').split('=')[1]) || 2;
const maxGK = Number((argv.find(a=> a.startsWith('--maxgk='))||'--maxgk=3').split('=')[1]) || 3;

const formations = parseFormations(formationArg);
// Compute average weights across provided formations
const weightDef = Math.round(formations.reduce((a,f)=> a+f.DEF,0) / formations.length * 100) / 100;
const weightMid = Math.round(formations.reduce((a,f)=> a+f.MID,0) / formations.length * 100) / 100;
const weightAtt = Math.round(formations.reduce((a,f)=> a+f.ATT,0) / formations.length * 100) / 100;

console.log('Formations:', formationArg, '->', formations);
console.log('Target roster size:', targetTotal, 'minGK:', minGK, 'maxGK:', maxGK, 'apply:', apply);
...
for(const team of Object.keys(rosters)){
  ...
  while(groups.GK.length > maxGK){
    ...
    groups.GK = []; groups.DEF=[]; groups.MID=[]; groups.ATT=[];
    players.forEach((p, idx)=>{ const g=posToGroup(p.position||p.pos); groups[g].push({ idx, player: p }); });
  }
  ...
  if(remaining > 0){
    ...
    const desiredFloor = { DEF: Math.floor(desiredFloat.DEF), MID: Math.floor(desiredFloat.MID), ATT: Math.floor(desiredFloat.ATT) };
    assigned = desiredFloor.DEF + desiredFloor.MID + desiredFloor.ATT;
    const remainders = [ ['DEF', desiredFloat.DEF - desiredFloor.DEF], ['MID', desiredFloat.MID - desiredFloor.MID], ['ATT', desiredFloat.ATT - desiredFloor.ATT] ];
  ...
  const desiredCounts = { GK: desiredGK, DEF: desired.DEF, MID: desired.MID, ATT: desired.ATT, CM: desiredMid.CM, LW: desiredMid.LW, RW: desiredMid.RW };
  ...
  while(players.length > targetTotal){
    ...
    const counts = { GK: groups.GK.length, DEF: groups.DEF.length, MID: groups.MID.length, ATT: groups.ATT.length };
    ...
    const tied = Object.keys(norm).filter(k=> Math.abs(norm[k] - norm[pick]) < 1e-9 && removable.includes(k));
    if(tied.length > 1){
      ...
      if(tied.length > 1 && Math.abs(surplus[tied[0]] - surplus[tied[1]]) < 1e-9){
        ...
        for(const o of order) if(tied.includes(o)) { pick = o; break; }
      }
    }
    ...
    if(pick === 'MID'){
      ...
      groups.MID.forEach(e=>{ const sub = posToSubMid(e.player.position||e.player.pos); midSubs[sub].push(e); });
      const subCounts = { CM: midSubs.CM.length, LW: midSubs.LW.length, RW: midSubs.RW.length };
      const subDesired = { CM: desiredCounts.CM || 0, LW: desiredCounts.LW || 0, RW: desiredCounts.RW || 0 };
      const subSurplus = { CM: subCounts.CM - subDesired.CM, LW: subCounts.LW - subDesired.LW, RW: subCounts.RW - subDesired.RW };
      const subNorm = {};
    ...
    if(!grpArr || grpArr.length===0){
      ...
      const idx = players.map(p=>p.name+'|'+(p.position||p.pos)).lastIndexOf(rem.player.name+'|'+(rem.player.position||rem.player.pos));
...
// Summary
console.log('Proposed removals:', actions.length);
const byTeam = {};
actions.forEach(a=>{ byTeam[a.team] = byTeam[a.team] || []; byTeam[a.team].push(a); });
Object.keys(byTeam).forEach(t=>{
  console.log(`\nTeam: ${t}`);
  byTeam[t].forEach(a=>{
    console.log(` - ${a.action} -> ${a.removed.name} (${a.removed.position||a.removed.pos||''}) ${a.group? '['+a.group+']':''}`);
  });
});
