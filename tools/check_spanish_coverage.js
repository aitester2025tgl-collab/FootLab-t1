...
const keymap={}; keys.forEach((k,i)=> keymap[norm(k)]={name:k,pos:i+1});
console.log('Total teams in file:',keys.length);
expected.forEach(e=>{
  const n=norm(e);
  if(keymap[n]) console.log(e,'-> FOUND as "'+keymap[n].name+'" at position',keymap[n].pos);
  else{
    const found=keys.find(k=> norm(k).includes(n) || n.includes(norm(k)));
    if(found) console.log(e,'-> fuzzy match as "'+found+'" at position', keys.indexOf(found)+1);
    else console.log(e,'-> MISSING');
  }
});
